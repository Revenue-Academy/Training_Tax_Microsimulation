...
"Calculation for minimum base salary"
@iterate_jit(nopython=True)
def cal_min_base_wage(average_gross_wage, min_base_wage):
    min_base_wage=(average_gross_wage*0.5)*12
    return min_base_wage

"Calculation for maximum base salary"
@iterate_jit(nopython=True)
def cal_max_base_wage(average_gross_wage ,max_base_wage):
    max_base_wage= ((average_gross_wage)*16)*12
    return max_base_wage

"Calculation for max base salary for people for company vs self-employment /(16 vs 12)"

"Calculation for minimum ssc"
@iterate_jit(nopython=True)
def cal_min_ssc (min_base_wage,ssc_rate_w):
    min_ssc = min_base_wage*ssc_rate_w
    return min_ssc

"Calculation for maximum ssc"
@iterate_jit(nopython=True)
def cal_max_ssc (max_base_wage,ssc_rate_w):
    max_ssc = max_base_wage*ssc_rate_w
    return max_ssc

"Calculation for ssc"  
@iterate_jit(nopython=True)
def cal_ssc_w(ssc_w, income_wage_l, ssc_rate_w, average_gross_wage, min_base_wage, max_base_wage, min_ssc, max_ssc, ssc_w_calc):
    """
    ...
    """
    if ssc_w ==0 & income_wage_l> 1:
        ssc_w_calc = 0
    elif income_wage_l <min_base_wage:
           ssc_w_calc = min_ssc
    elif (income_wage_l >=min_base_wage) &( income_wage_l <=max_base_wage):
            ssc_w_calc = ssc_rate_w*income_wage_l
    elif income_wage_l > max_base_wage:
            ssc_w_calc = max_ssc
    return ssc_w_calc

"Calculation for personal allowance policy variable "
@iterate_jit(nopython=True)
def cal_personal_allowance_change(rate_personal_allowance_w,
                                  personal_allowance_w, personal_allowance_change):
    personal_allowance_change = personal_allowance_w*rate_personal_allowance_w
    return(personal_allowance_change)
...
def cal_expenditure_w (pit_w, te_disable_unemp, te_special_comp, te_TIDZ, te):
    if te_disable_unemp==1 or te_special_comp==1 or te_TIDZ==1:
        te = pit_w
    elif te_disable_unemp==0 or te_special_comp==0 or te_TIDZ==0:
        te==0
        return(te) 
    
"Calculation for tax base for other labor income" 
@iterate_jit(nopython=True)
def cal_tax_base_other(deduction_income_agr_med_l, income_add_l, income_supvr_l, income_officials_l, 
                       income_jury_l, income_manu_l, income_contract_l, 
                       income_agr_med_l, tax_base_other):
    tax_base_other = (income_add_l+ income_supvr_l+ income_officials_l, income_jury_l + 
                      income_manu_l + income_contract_l + (income_agr_med_l-(deduction_income_agr_med_l*income_agr_med_l)))
    tax_base_other = max(tax_base_other, 0)
    return tax_base_other
                       
 
"Calculation for total tax base from labor"
@iterate_jit(nopython=True)
...
"Calculation for PIT from labor"
@iterate_jit(nopython=True)
...
"Calculation for tax base from capial for those sources without deductions"
@iterate_jit(nopython=True)
def cal_tti_wd(income_dividends_c, income_interest_c, tti_wd):
    tti_wd = (income_dividends_c + income_interest_c)
    return (tti_wd)

"Calculation for tax base capital from income from property/rental income"
@iterate_jit(nopython=True)
def cal_deduction_rental_income_c(rate_rent_house_business_c,rate_rent_non_house_business_c,rent_house_business,
                                  ...
                                  rent_actual_cost,income_prop_c,deduction_rental_income_c):
    if rent_house_business :
        deduction_rental_income_c= rate_rent_house_business_c * income_prop_c
    elif rent_non_house_business :
        deduction_rental_income_c = rate_rent_non_house_business_c * income_prop_c 
    elif rent_actual_costing :
        deduction_rental_income_c = rent_actual_cost
        
    return(deduction_rental_income_c)
...
def cal_deduction_copyrights_income_c(rate_income_sculpture_c,rate_income_artistic_photography_c,
                                      ...
                                      income_copyrights_c, deduction_copyrights_income_c):
    if income_sculpture:
        deduction_copyrights_income_c= rate_income_sculpture_c * income_copyrights_c
    elif income_artistic_photography :
        deduction_copyrights_income_c = rate_income_artistic_photography_c * income_copyrights_c
    elif income_paintings:
        deduction_copyrights_income_c = rate_income_paintings_c * income_copyrights_c
    elif income_music_ballet:
        deduction_copyrights_income_c = rate_income_music_ballet_c * income_copyrights_c
    elif income_translations_lectures:
        deduction_copyrights_income_c = rate_income_translations_lectures_c * income_copyrights_c
    elif income_stage_music:
        deduction_copyrights_income_c = rate_income_stage_music_c * income_copyrights_c
    elif income_copyrights_other:
        deduction_copyrights_income_c = rate_income_copyrights_other_c * income_copyrights_c
    elif income_copyrights_actual_costing :
        deduction_copyrights_income_c = income_copyrights_actual_cost
        return(deduction_copyrights_income_c)    

"Calculation for tax base for capital for other capital income"
@iterate_jit(nopython=True)
def cal_deduction_income_other_c(rate_income_claimed_other_c, income_claimed_other_c, income_other_c,
                                 deductions_income_other_c):
    if income_claimed_other_c: 
        deduction_income_other_c = rate_income_claimed_other_c*income_other_c
        return (deduction_income_other_c)
    
"Calculation for total tax base from capital income except games of chance"
@iterate_jit(nopython=True)
def cal_tti_c_a(tti_wd, deduction_rental_income_c, deduction_copyrights_income_c, deduction_income_other_c, tti_c_a):
    tti_c_a = tti_wd + deduction_rental_income_c + deduction_copyrights_income_c + deduction_income_other_c
...
#(income_prop_c-deductions_income_prop_c)+(income_copyrights_c-deductions_income_copyrights_c)+
#(income_other_c-deductions_income_other_c)
...
@iterate_jit(nopython=True)
def cal_pit_c(capital_income_rate_a, capital_income_rate_g, tti_c_a, tti_c_g, pit_c):
    pit_c = (tti_c_a*capital_income_rate_a) + (tti_c_g*capital_income_rate_g)
    return pit_c

#Have to confirm with Jordan the different rates for all other capital income and games of chance
#Not sure how to add behaviour component in the above case
'''
"Calculation for PIT for capital"
@iterate_jit(nopython=True)
...
@iterate_jit(nopython=True)
def cal_net_w_I(tti_w_I, pit_w_I):
    """
...
@iterate_jit(nopython=True)
def cal_total_net_income(net_w_I, net_i_c, total_net_income):
    """
    Compute net income.
    """
    total_net_income = net_w_I + net_i_c
