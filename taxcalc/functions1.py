...
def net_salary_income(SALARY, std_deduction, Income_Salary):
    ...
    introduced only from AY 2019 onwards, "std_deduction" is set as 30000 for
...
@iterate_jit(nopython=True)
def taxable_income(TTI, TI_special_rates):
    """
    Calculates the total capital gains and tax on it
    which are taxed at spl rates
    """
...
def pit_liability(rate1, rate2, rate3, rate4, tbrk1, tbrk2, tbrk3, tbrk4,
    ...
    agginc = Aggregate_Income + Income_Rate_Purpose
    agginc = max(0., agginc)
    ...
    # Agricultural rebate only applicable if taxinc is greater than tbrk1
    if Aggregate_Income <= tbrk1:
        rebate_agri = 0.
    ...
    return (tax_Aggregate_Income, rebate_agri, tax_TTI,
